...
// Mobile-first responsive animation utilities
...
export class AnimationUtils {
  ...
  static createAnimation(
    ...
    return element.animate(keyframes, {
      ...options,
      duration: this.getAnimationDuration(options.duration as number)
    });
  }
   ...
   */
  static applyResponsiveAnimation(
    element: Element,
    ...
    if (!config.duration || !config.easing) return null;
  ...
  static createEntranceAnimation(
    element: Element,
    type: keyof typeof ANIMATION_PRESETS = 'fadeIn',
  ...
  static createExitAnimation(
    element: Element,
    type: keyof typeof ANIMATION_PRESETS = 'fadeOut',
    options?: Partial<AnimationConfig>
  ): Animation | null {
    const preset = ANIMATION_PRESETS[type];
    const keyframes = Object.entries(preset.keyframes).map(([key, value]) => ({
