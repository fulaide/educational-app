...
__export(index_exports, {
  ...
  PrismaClient: () => import_client3.PrismaClient,
  SAMPLE_DATA: () => SAMPLE_DATA,
  SEED_CONFIGS: () => SEED_CONFIGS,
  applyMigrations: () => applyMigrations,
  createBackup: () => createBackup,
  createMigration: () => createMigration,
  getSeedConfig: () => getSeedConfig,
...
var DatabaseUtils = class {
  ...
  static async backup(name) {
    ...
    const backupPath = import_path.default.join(this.backupDir, `${backupName}.sql`);
    return new Promise((resolve, reject) => {
      ...
      const pgDump = (0, import_child_process.spawn)("pg_dump", [
        "-h",
        url.hostname,
        "-p",
        url.port || "5432",
  ...
  static async restore(backupPath) {
    return new Promise((resolve, reject) => {
      ...
      const url = new URL(databaseUrl);
      const psql = (0, import_child_process.spawn)("psql", [
        "-h",
        url.hostname,
        "-p",
        url.port || "5432",
  ...
  static async listBackups() {
    try {
      await this.ensureBackupDir();
      const files = await import_promises.default.readdir(this.backupDir);
  ...
  static async cleanOldBackups(keepCount = 5) {
    try {
      ...
      for (const backup of toDelete) {
        await import_promises.default.unlink(import_path.default.join(this.backupDir, backup));
...
var MigrationUtils = class {
  ...
  static async getAllMigrations() {
    try {
      const migrations = await import_promises2.default.readdir(this.migrationDir);
  ...
  static async validateMigrations() {
    ...
    try {
      try {
        await import_promises2.default.access(this.migrationDir);
      } catch {
      ...
      for (const migration of allMigrations) {
        const migrationPath = import_path2.default.join(this.migrationDir, migration);
        try {
          const stat = await import_promises2.default.stat(migrationPath);
          if (!stat.isDirectory()) {
            issues.push(`Migration ${migration} is not a directory`);
          } else {
            const migrationSqlPath = import_path2.default.join(migrationPath, "migration.sql");
            try {
              await import_promises2.default.access(migrationSqlPath);
            } catch {
