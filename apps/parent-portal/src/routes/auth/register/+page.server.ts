...
import { SUPPORTED_LOCALES, DEFAULT_LOCALE } from '@educational-app/i18n'

const JWT_SECRET = process.env.AUTH_SECRET || 'dev-secret-key'

// Helper function to detect user's preferred locale
function detectUserLocale(request: Request): string {
	...
	if (acceptLanguage) {
		...
		const languages = acceptLanguage
			...
			.map(lang => lang.split('-')[0]) // Get just the language code
...
export const actions: Actions = {
	default: async ({ request, cookies, url }) => {
		...
		console.log('[REGISTER] Form data received:', { email, name, hasPassword: !!password, hasConfirmPassword: !!confirmPassword })
		...
		try {
			...
			if (existingUser) {
				console.log('[REGISTER] User already exists with role:', existingUser.role)
				if (existingUser.role === 'PARENT') {
					return fail(400, { 
						error: 'A parent account with this email already exists. Please sign in instead.', 
						email, 
						name 
					})
				} else {
					return fail(400, { 
						error: `An account with this email already exists as a ${existingUser.role.toLowerCase()}. Please use a different email address or contact support if you need to access multiple portals.`, 
						email, 
						name 
					})
				}
			...
			console.log('[REGISTER] Detected user locale:', userLocale)
			...
			console.log('[REGISTER] Created new parent account:', email)
			...
			// Send parent-specific verification email with dynamic URL
			...
			console.log('[REGISTER] Returning success result:', result)
			return result
