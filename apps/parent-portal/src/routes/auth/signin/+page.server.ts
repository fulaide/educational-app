...
export const actions: Actions = {
	default: async ({ request, cookies }) => {
		...
		try {
			...
			// Check if user is a parent (only parents can access parent portal)
			if (user.role !== 'PARENT') {
				return fail(400, { error: 'This portal is only for parents. Please use the appropriate portal for your role.' })
			}
			...
			const userData = {
				...
				locale: (user.settings as any)?.locale || 'de' // Get locale from user settings
			}
